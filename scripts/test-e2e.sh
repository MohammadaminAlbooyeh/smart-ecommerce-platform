...
# Zarfi: infra (docker compose up) + service-ha run hastan. Baraye payment, az kafka-console-producer
# tooye container-e kafka estefade mikonim.
...
ORDER_ID=$(for i in $(seq 1 20); do
  curl -sf "$ORDER_URL/api/orders?userId=$DEMO_USER" 2>/dev/null | python3 -c "
import sys,json
try:
